...
set, the deadline bounds the optional database describe check on connect, and
the `WriteRecords`/`CreateTable` call(s) made for one write.

When a write is cancelled, the delivery outcome of the in-flight batch is
unknown: Timestream may or may not have received some or all of the records.
